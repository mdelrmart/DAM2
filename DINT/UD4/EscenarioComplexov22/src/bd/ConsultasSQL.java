...
public class ConsultasSQL {
  ...
  public static int eliminarUnPropietarioDadoSeuDni(String dni)
    ...
    public static ArrayList recuperarTodolosCansDunPropietario(String dniPropietario)
    {
      ...
      public static void eliminarTodolosCansDunPropietario(String dniPropietario)
    {
        try
        {
            Statement stmt = Conexion.getConexion().createStatement();
            String consulta= "delete from cans where dniPropietario='"+dniPropietario+"'";
            //System.out.println(consulta);
            stmt.executeUpdate(consulta);
    ...
    public static void asiganrMascotasANovoPropietario(String dniPropietrioAntigo, String dniNovoProietario) {
        try
        {
            Statement stmt=Conexion.getConexion().createStatement();
            String consulta= "update cans set dniPropietario= '" + dniNovoProietario + "' where dniPropietario='"+ dniPropietrioAntigo +"'";
            stmt.executeUpdate(consulta);            
        } catch(SQLException e)
        {
            e.printStackTrace();
    ...
    public static int existeVacinacionDeDoseSuperior(int codVacina, int dose, String chip)
    {
     ...
     public static ArrayList recuperarIdFacturas(String strWhere) 
    {
          try {
            ...
            while(rs.next())
            {
                 resultado.add(rs.getInt("id_factura"));
            }
            return resultado;  
        } catch (SQLException e) {
             e.printStackTrace();
      ...
      // Recuperar todos los propietarios
    public static ArrayList recuperarTodosOsPropietarios()
    {
        try
        {
            ArrayList resultado=new ArrayList();
            Statement stmt = Conexion.getConexion().createStatement();
            String consulta= "select dni, nome, ap1, ap2, tlf, email from propietarios";
            //System.out.println(consulta);
            ResultSet rs = stmt.executeQuery(consulta);
            while(rs.next())
            {
